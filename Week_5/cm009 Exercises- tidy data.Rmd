...
Yes, the data has one observation and 1 valuable for each variable 
2. Widen the data so that we see the words spoken by each race, by putting race as its own column.

Notes (from handwritten): Why untidy data is bad: hard to do pairwise analysis 

```{r}
(lotr_wide <- lotr %>% 
  pivot_wider(id_cols= c(-Race, -Words), ## identifiers
              names_from= Race, 
              values_from = Words))
## These are new functions to widen/lengthen data-->pivoting functions
```
...
## need to include . for value not just "value"
```
