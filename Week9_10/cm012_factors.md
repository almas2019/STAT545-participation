...
gapminder %>% 
  ...
  mutate(life_level = case_when(lifeExp < 23 ~ "very low",
                                ...
                                lifeExp < 59 ~ "moderate",
                                lifeExp < 70~ "high",
                                TRUE ~ "very high")) %>% ## TRUE= for every other value not meeting this condition like the else
  ggplot() + geom_boxplot(aes(x = life_level, y =gdpPercap)) +
  labs(y = "GDP per capita, $", x= "Life expectancy level, years") +
  theme_bw() 
```

![](cm012_factors_files/figure-html/unnamed-chunk-1-1.png)<!-- -->

Do you notice anything odd/wrong about the graph?
Since there are no countries that met this observation (very low), R did not include it, but we created 
...
gapminder %>% 
  ...
  ## TRUE= for every other value not meeting this condition like the else
  ggplot() + geom_boxplot(aes(x = life_level, y =gdpPercap)) +
  labs(y = "GDP per capita, $", x= "Life expectancy level, years") +
...
![](cm012_factors_files/figure-html/unnamed-chunk-2-1.png)<!-- -->

Good to drop : Only look at data that you have
Bad to drop factors: want to know if there are no data in categories, visually harder to compare 
...
```r
gap <- gapminder %>%
  filter(country %in% c("Canada","Japan","Egypt","Pakistan","New Zealand"),) 
gap$country %>%
  nlevels()
```
...
gapminder %>%
  ...
  coord_flip() + #flips x axis onto your y
  theme_bw() +
  ylab("Number of entries") + xlab("Continent")
```

![](cm012_factors_files/figure-html/unnamed-chunk-7-1.png)<!-- -->
...
![](cm012_factors_files/figure-html/unnamed-chunk-8-1.png)<!-- -->
...
gapminder %>%
  ggplot() +
  geom_bar(aes(fct_reorder(continent,lifeExp,max))) + ## reorder the function based on maximum life Exp
  coord_flip()+
  theme_bw() +
  ylab("Number of entries") + xlab("Continent")
```

![](cm012_factors_files/figure-html/unnamed-chunk-9-1.png)<!-- -->
...
gapminder %>%
  ggplot() +
  geom_bar(aes(fct_relevel(continent,"Oceania","Americas"))) + ## reorder the function based on maximum life Exp
  coord_flip()+
  theme_bw() +
  ylab("Number of entries") + xlab("Continent")
```

![](cm012_factors_files/figure-html/unnamed-chunk-11-1.png)<!-- -->
...
gapminder %>%
  ggplot() +
  geom_bar(aes(fct_recode(continent,"OCN"="Oceania","AME"="Americas"))) + ## reorder the function based on maximum life Exp
  coord_flip()+
  theme_bw() +
  ylab("Number of entries") + xlab("Continent")
```

![](cm012_factors_files/figure-html/unnamed-chunk-12-1.png)<!-- -->
